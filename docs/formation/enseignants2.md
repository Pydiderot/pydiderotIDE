...
Les documents qui se trouvent dans : `Atrium > Site des professeurs de maths > Documents > Python > Mutualisation` se répartissent ainsi : il y a des documents généraux, des fiches techniques, des activités à faire avec les élèves (certaines avec Python, d'autres sans Python). 
...
Pour y aller en douceur, on peut par exemple faire une première activité "débranchée" (mais cela peut sembler inutile lorsqu'ils ont déjà travaillé ce genre de chose au collège...).
