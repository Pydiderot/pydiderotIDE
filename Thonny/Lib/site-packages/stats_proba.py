...
def compte(liste_critere,liste_effectif=[]):
    """
    Retourne la liste triée sans les doublons, les effectifs, les fréquences correspondantes et l'effectifs totales.

    Arguments:
        liste_critere (list): une liste de critere d'une population
    ...
    L=[]
    for i in range(len(critere_groupe)):
        L.append([critere_groupe[i],eff_groupe[i]])

    L.sort()
    #On renvoie comme il faut
    critere=[i for i,j in L]
    effectif=[j for i,j in L]
    frequence=[float(j)/eff_tot for i,j in L]

    return critere,effectif,frequence,eff_tot
