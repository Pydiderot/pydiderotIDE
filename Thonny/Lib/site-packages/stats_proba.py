...
import random
from listes import trier_liste
...
def tirage_binomial(n, p):
    """
    Renvoie un nombre entier (int) choisi de manière aléatoire selon une loi binomiale B(n,p).
...
def intervalle(debut, fin, pas=1):
    ...
    Arguments:
        ...
        pas (int, optionel): Le pas (``1`` par défaut).
    """
...
def baton(xi, ni='optionnel', couleur='b'):
    ...
    Arguments:
        ...
        couleur (str, optionel):  la couleur du diagramme (??? par défaut)
    """
...
def histop(Liste, Classes='optionnel'):  # Majuscule sur une variable est mauvais style.
    ...
    Arguments:
        ...
        Classes (int ou list, optionel): voir ci dessus
    """
    C = Classes
    if C == 'optionnel':
        C = 10
...
def compte(liste, option='optionnel'):
    ...
    Arguments:
        ...
        option (str, optionel): "frequence" ou "effectif"
    """
...
def polygoneECC(xi, ni='optionnel', couleur='b'):
    ...
    Arguments:
        xi (list): une liste de valeurs
        ni (list, optionel): la liste des effectifs associés.
        couleur (str, optionel): couleur du polygone ('b' par défaut)
...
def polygoneFCC(xi, ni='optionnel', couleur='b'):
    ...
    Arguments:
        xi (list): liste de valeurs.
        ni (list, optionel): liste des effectifs
        couleur (str, optionel): couleur de graphique (``'b'`` par défaut)
    """
    xi, ec = FCC(xi, ni)
    if len(xi) == len(ec) + 1:
        ec.insert(0, 0)
...
def moyenne(xi, ni='optionnel'):
    ...
    Arguments:
        ...
        ni (liste, optionel): série des effectifs associés
    """
    if ni == 'optionnel':
        xi, ni = compte(xi, 'effectif')
    if len(xi) == len(ni) + 1:
